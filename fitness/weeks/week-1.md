# Week 1: Seattle Baseline

**Focus:** Establish baseline while traveling
**One behavior:** Do 2 short workouts + PT stretches
**Success metric:** 60+ minutes total

---

## The Principle

You're traveling. The goal isn't to crush it - it's to **not lose momentum**.

Two short sessions + PT stretches proves you can work out anywhere. That's the win.

---

## The Sessions

### Session 1: Upper (Bodyweight)
**When:** Day 2-3 in Seattle
**Duration:** 20 min
**Location:** Brother's house

```
Warmup (3 min)
- Arm circles, shoulder rolls

Pushups: 3 sets × max (write down the number)
Rows: 3 sets × 10 (use a sturdy table edge or heavy bag)
Dips: 2 sets × 8 (use a chair)
Plank: 2 × 20 sec

Cool down (2 min)
```

### Session 2: Mid (PT Focus)
**When:** Day 5-6 in Seattle
**Duration:** 15 min
**Location:** Anywhere

```
Dead bugs: 3 sets × 10 each side
Bird dogs: 3 sets × 10 each side
Glute bridges: 2 sets × 12
Cat-cow: 2 sets × 10
Child's pose: 1 min hold
```

### PT Stretches
**When:** 2 times during the week
**Duration:** 15 min each

Your regular PT routine. These count toward minutes.

---

## Weekly Schedule Template

| Day | Activity | Duration |
|-----|----------|----------|
| Thu (Jan 2) | PT stretches | 15 min |
| Fri (Jan 3) | Rest / travel prep | - |
| Sat (Jan 4) | Rest / travel prep | - |
| Sun (Jan 5) | Travel to Seattle | - |
| Mon (Jan 6) | Upper session | 20 min |
| Tue (Jan 7) | PT stretches | 15 min |
| Wed (Jan 8) | Mid session | 15 min |

**Total:** 65 min

---

## Session Tracking

| Session | Planned | Done? | Notes |
|---------|---------|-------|-------|
| Upper (20 min) | Mon Jan 6 | [ ] | Pushup max: ___ |
| Mid (15 min) | Wed Jan 8 | [ ] | |
| PT Stretches #1 | Thu Jan 2 | [ ] | |
| PT Stretches #2 | Tue Jan 7 | [ ] | |

---

## If You Miss a Session

**Don't spiral.** You're traveling. One session + PT stretches is still a win.

If you miss Upper:
- Do it the next day, or
- Swap for a 15-min bodyweight circuit

If you miss Mid:
- Just do PT stretches - they cover similar ground

---

## Equipment Needed

Pack these (or find at brother's):
- [ ] Comfortable workout clothes
- [ ] Resistance band (optional, useful for rows)
- [ ] Your PT stretch notes (if you have specific exercises)

---

## End of Week Reflection

Answer before Week 2:

1. **Compliance:** How many sessions did I complete? ___/4

2. **What worked?**
   - [ ] Morning sessions
   - [ ] Evening sessions
   - [ ] Combining with something else
   - [ ] Other: _______________

3. **What made it hard?**
   - [ ] Forgot
   - [ ] Felt weird working out at brother's
   - [ ] No energy after travel
   - [ ] Other: _______________

4. **Pushup baseline:** Max reps in set 1: ___

5. **Adjustment for Week 2:**

---

## Level Progress

| Metric | Start of Week | End of Week |
|--------|---------------|-------------|
| Weekly minutes | ~50 | 65 (target) |
| Sessions/week | sporadic | 2 |
| Pushup max | ~10 | ___ |

---

## Connection to Goal

This week proves: **You can work out while traveling.**

```
Week 1: Baseline in Seattle (you are here)
   ↓
Week 2: Add variety (jog)
   ↓
Week 3: Back in NYC, first swim
   ↓
...
Week 8: 200+ minutes
```

Don't worry about perfect. Just don't go to zero.

---

[← Back to Fitness Overview](../) | [Next Week →](week-2)
