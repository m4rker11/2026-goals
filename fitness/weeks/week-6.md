# Week 6: Full Routine

**Focus:** Lock in the complete weekly pattern
**One behavior:** 3 gym + Bollywood + PT
**Success metric:** 180+ minutes total

---

## The Principle

This is what your "normal week" looks like.

3 gym sessions. 1 Bollywood class. PT stretches. **This is the routine.**

---

## The Sessions

### Session 1: Upper - Push
**Duration:** 35 min
**Location:** Gym

```
Warmup (5 min)

Pushups: 4 sets × max (you should be improving)
Dips: 3 sets × 10
Shoulder press: 3 sets × 10
Chest fly: 2 sets × 12
Tricep work: 2 sets × 12

Cool down (2 min)
```

### Session 2: Upper - Pull
**Duration:** 35 min
**Location:** Gym

```
Warmup (5 min)

Pullups: 4 sets × max
Dumbbell rows: 3 sets × 12 each
Face pulls: 3 sets × 15
Lat pulldown: 2 sets × 12
Bicep curls: 3 sets × 12

Cool down (2 min)
```

### Session 3: Cardio (Zone 2)
**Duration:** 25 min
**Location:** Gym

```
Pick one:
- Rowing: 25 min steady
- Bike: 25 min steady
- Incline treadmill: 25 min

Zone 2 = can hold a conversation
This builds your VO2 max base
```

### Session 4: Bollywood Dance
**Duration:** 60 min

### Session 5: Blink with Kai
**When:** Weekend
**Duration:** 45 min
**Location:** Blink Fitness

```
Buddy sessions are now routine
```

### PT Stretches
**When:** 2 times during the week
**Duration:** 15 min each

---

## Weekly Schedule Template

| Day | Activity | Duration |
|-----|----------|----------|
| Thu (Feb 6) | Upper Push | 35 min |
| Fri (Feb 7) | Rest | - |
| Sat (Feb 8) | Blink with Kai | 45 min |
| Sun (Feb 9) | Bollywood | 60 min |
| Mon (Feb 10) | Upper Pull | 35 min |
| Tue (Feb 11) | Cardio | 25 min |
| Wed (Feb 12) | PT stretches | 30 min |

**Total:** 230 min

---

## Session Tracking

| Session | Planned | Done? | Notes |
|---------|---------|-------|-------|
| Upper Push (35 min) | Thu Feb 6 | [ ] | Pushups: ___ |
| Blink with Kai (45 min) | Sat Feb 8 | [ ] | |
| Bollywood (60 min) | Sun Feb 9 | [ ] | |
| Upper Pull (35 min) | Mon Feb 10 | [ ] | Pullups: ___ |
| Cardio (25 min) | Tue Feb 11 | [ ] | Type: ___ |
| PT Stretches | Wed Feb 12 | [ ] | 30 min |

---

## If You Miss a Session

Priority order:
1. Bollywood (it's fun, don't skip fun)
2. Upper Push or Pull (track your numbers)
3. Cardio (can be replaced with longer warm-up in other sessions)

Missing one gym session is fine. Just make sure you hit 2 + Bollywood.

---

## Pattern Recognition

You now have a repeatable week:

```
Mon/Tue: Cardio or rest
Wed: Rest
Thu: Upper Push
Fri: Rest
Sat: Bollywood
Sun: Upper Pull
```

Adjust based on when Bollywood is actually scheduled.

---

## End of Week Reflection

1. **Compliance:** Sessions completed: ___/6

2. **This felt:**
   - [ ] Like my new normal
   - [ ] Still figuring it out
   - [ ] Overwhelming

3. **Best session this week:**
   - [ ] Upper Push
   - [ ] Upper Pull
   - [ ] Cardio
   - [ ] Bollywood
   - Why? _______________

4. **Key numbers:**
   - Pushups set 1: ___
   - Pullups set 1: ___

5. **Approaching 200 min - how does that feel?**

---

## Level Progress

| Metric | Start of Week | End of Week |
|--------|---------------|-------------|
| Weekly minutes | 155 | 180 (target) |
| Gym sessions | 2 | 3 |
| Routine status | forming | locked |

---

## Connection to Goal

This week establishes: **Your complete weekly routine**

```
Week 1-5: Building up ✓
Week 6: Full routine locked (you are here)
   ↓
Week 7: Building duration
   ↓
Week 8: 200+ achieved
```

---

[← Back to Fitness Overview](../) | [← Previous Week](week-5) | [Next Week →](week-7)
