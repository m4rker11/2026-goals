# Week 4: Three Gym Sessions

**Focus:** First week with 3 gym sessions
**One behavior:** Complete 3 distinct gym days
**Success metric:** 145+ minutes total

---

## The Principle

Two gym sessions felt okay. Now we add a third.

This is where the routine starts to form. **Three is a pattern.**

**New this week:** Gym buddy sessions with Kai at Blink on weekends. Accountability makes it easier.

---

## The Sessions

### Session 1: Upper - Push Focus
**Duration:** 30 min
**Location:** Gym

```
Warmup (5 min)

Pushups: 4 sets × max
Dips: 3 sets × 8-10
Shoulder press: 3 sets × 10
Tricep pushdowns: 2 sets × 12
Chest fly (if available): 2 sets × 12

Cool down (2 min)
```

### Session 2: Upper - Pull Focus
**Duration:** 30 min
**Location:** Gym

```
Warmup (5 min)

Pullups: 4 sets × max
Dumbbell rows: 3 sets × 10 each
Face pulls: 2 sets × 15
Bicep curls: 3 sets × 12
Lat pulldown (if available): 2 sets × 12

Cool down (2 min)
```

### Session 3: Mid + Cardio
**Duration:** 30 min
**Location:** Gym

```
Core (15 min):
Dead bugs: 3 sets × 12 each
Plank: 3 × 30 sec
Bird dogs: 3 sets × 10 each
Pallof press: 2 sets × 10 each (if cable available)

Cardio (15 min):
- Rowing, bike, or incline treadmill
- Zone 2 pace
```

### Session 4: Blink with Kai
**When:** Weekend
**Duration:** 45 min
**Location:** Blink Fitness

```
Whatever Kai wants to do + your exercises
- Having a buddy makes showing up easier
- Mix of both your routines
- Social motivation counts
```

### PT Stretches
**When:** 2 times during the week
**Duration:** 15 min each

---

## Weekly Schedule Template

| Day | Activity | Duration |
|-----|----------|----------|
| Thu (Jan 23) | Upper - Push | 30 min |
| Fri (Jan 24) | Rest | - |
| Sat (Jan 25) | Blink with Kai | 45 min |
| Sun (Jan 26) | Upper - Pull | 30 min |
| Mon (Jan 27) | Mid + Cardio | 30 min |
| Tue (Jan 28) | PT stretches | 15 min |
| Wed (Jan 29) | PT stretches | 15 min |

**Total:** 135 min gym + 30 min PT = **165 min**

---

## Session Tracking

| Session | Planned | Done? | Notes |
|---------|---------|-------|-------|
| Upper Push (30 min) | Thu Jan 23 | [ ] | Pushups: ___ |
| Blink with Kai (45 min) | Sat Jan 25 | [ ] | First buddy session! |
| Upper Pull (30 min) | Sun Jan 26 | [ ] | Pullups: ___ |
| Mid + Cardio (30 min) | Mon Jan 27 | [ ] | |
| PT Stretches #1 | Tue Jan 28 | [ ] | |
| PT Stretches #2 | Wed Jan 29 | [ ] | |

---

## If You Miss a Session

**Three sessions is new.** If you only hit 2, that's still okay.

Priority order:
1. Upper Push (track pushup progress)
2. Upper Pull (track pullup progress)
3. Mid + Cardio (can skip if needed)

---

## Why Push/Pull Split This Week

Instead of one "Upper" day, we split into Push and Pull:
- More focused work on each movement pattern
- Better for building pushup/pullup numbers
- Sets you up for calisthenics progression

---

## End of Week Reflection

1. **Compliance:** Gym sessions completed: ___/3

2. **Three sessions felt:**
   - [ ] Totally manageable
   - [ ] A stretch but doable
   - [ ] Too much

3. **30 min sessions felt:**
   - [ ] Too short
   - [ ] Just right
   - [ ] Still learning to fill the time

4. **Key numbers:**
   - Pushups set 1: ___ (vs last week: ___)
   - Pullups set 1: ___ (vs last week: ___)

5. **Bollywood starts next week - ready?**

---

## Level Progress

| Metric | Start of Week | End of Week |
|--------|---------------|-------------|
| Weekly minutes | 130 | 145 (target) |
| Gym sessions/week | 2 | 3 |
| Session types | Upper, Lower | Push, Pull, Mid+Cardio |

---

## Connection to Goal

This week establishes: **3 gym sessions is your new normal**

```
Week 1-3: Building foundation ✓
Week 4: Three gym sessions (you are here)
   ↓
Week 5: Bollywood begins
   ↓
Week 6: Full routine locked
   ↓
Week 8: 200+ minutes
```
